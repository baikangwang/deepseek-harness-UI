...
export function IdeSidebar(props: IdeSidebarProps): ReturnType<typeof createElement> {
  ...
  const buttons = (): Array<ReturnType<typeof createElement>> => views.map((v) => el('button', { key: v.id, type: 'button', title: v.label, 'aria-label': v.label, className: `dshide-activity-btn${props.wide && active === v.id ? ' active' : ''}`, onClick: () => { pick(v.id) } }, el(Icon, { name: v.icon, size: 20 })))

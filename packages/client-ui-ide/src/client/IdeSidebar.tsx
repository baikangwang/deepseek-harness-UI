...
export function IdeSidebar(props: IdeSidebarProps): ReturnType<typeof createElement> {
  ...
  const buttons = (): ReturnType<typeof createElement> => views.map((v) => el('button', { key: v.id, type: 'button', title: v.label, 'aria-label': v.label, className: `dshide-activity-btn${props.wide && active === v.id ? ' active' : ''}`, onClick: () => { pick(v.id) } }, el(Icon, { name: v.icon, size: 20 })))
  if (!props.wide) return el('div', { className: 'dshide-region rail' }, buttons())
  return el('div', { className: 'dshide-region' }, el('div', { className: 'dshide-activity' }, buttons()), el('div', { className: 'dshide-content' }, active === 'sessions' ? el(SessionView, { sessions: props.sessions, workspaces: props.workspaces, wsState, sessState }) : active === 'files' ? el(ExplorerView, { ide: props.ide, rpc: props.rpc, openDoc: props.openDoc, sessions: props.sessions, workspaces: props.workspaces, root, setRoot, workspaces: items }) : active === 'search' ? el(SearchView, { ide: props.ide, rpc: props.rpc, openDoc: props.openDoc, sessions: props.sessions, workspaces: props.workspaces, root }) : el(ScmView, { ide: props.ide, rpc: props.rpc, openDoc: props.openDoc, sessions: props.sessions, workspaces: props.workspaces, root })))
}

...
import { baseName, detectLang, renderLine } from "./lib.js";
function FileEditor(props) {
    ...
    useEffect(() => {
        let cancelled = false;
        setSt({ loading: true });
        call(ide.readText(props.path)).then((r) => { if (!cancelled)
            setSt({ content: r.content, error: '', truncated: r.truncated, loading: false }); }, (e) => { if (!cancelled)
            setSt({ error: String(e.message), loading: false }); });
        return () => { cancelled = true; };
    }, [props.path]);
    if (st.loading)
        return el('div', { className: 'dshide-loading' }, '加载中…');
    if (st.error)
        return el('div', { className: 'dshide-empty' }, st.error);
...
function DiffEditor(props) {
    ...
    useEffect(() => {
        let cancelled = false;
        setSt({ loading: true });
        call(ide.gitDiff(props.cwd, props.path)).then((r) => { if (!cancelled)
            setSt({ stdout: r.stdout, stderr: r.stderr, loading: false }); }, (e) => { if (!cancelled)
            setSt({ stderr: String(e.message), loading: false }); });
        return () => { cancelled = true; };
    }, [props.cwd, props.path]);
    if (st.loading)
        return el('div', { className: 'dshide-loading' }, '加载中…');
    if (st.stderr && !st.stdout)
        return el('div', { className: 'dshide-empty' }, st.stderr);

...
export function relTime(ts) {
    ...
    if (diff < m)
        return '刚刚';
    if (diff < h)
        return `${Math.floor(diff / m)} 分钟前`;
    if (diff < d)
        return `${Math.floor(diff / h)} 小时前`;
    if (diff < 30 * d)
...
export function detectLang(path) {
    ...
    if (/\.(js|jsx|mjs|cjs|ts|tsx)$/.test(p))
        return 'js';
    if (/\.json$/.test(p))
        return 'json';
    if (/\.(md|markdown)$/.test(p))
        return 'md';
    if (/\.(css|scss|less)$/.test(p))
        return 'css';
    if (/\.(html|htm|vue)$/.test(p))
        return 'html';
    if (/\.py$/.test(p))
        return 'py';
    if (/\.(sh|bash|zsh)$/.test(p))
        return 'sh';
    if (/\.ya?ml$/.test(p))
        return 'yaml';
    return 'text';
...
export function buildRules(lang) {
    if (lang === 'text')
        return [];
    if (lang === 'md')
        return [[/^#{1,6}[^\n]*/g, 'tok-md-heading'], [/`[^`\n]*`/g, 'tok-str'], [/\*\*[^*\n]+\*\*/g, 'tok-bold'], [/\[[^\]]*\]\([^)]*\)/g, 'tok-link'], [/^>\s?[^\n]*/g, 'tok-com']];
    if (lang === 'json')
        return [[/"(?:[^"\\]|\\.)*"(?=\s*:)/g, 'tok-json-key'], [/"(?:[^"\\]|\\.)*"/g, 'tok-str'], [/-?\b\d+(?:\.\d+)?(?:e[+-]?\d+)?\b/gi, 'tok-num'], [/\b(?:true|false|null)\b/g, 'tok-bool']];
    if (lang === 'html')
        return [[/<!--[\s\S]*?-->/g, 'tok-com'], [/<\/?[a-zA-Z][a-zA-Z0-9-]*/g, 'tok-tag'], [/\/?>/g, 'tok-tag'], [/[a-zA-Z-]+(?==")/g, 'tok-attr'], [/"[^"]*"/g, 'tok-str']];
    if (lang === 'css')
        return [[/\/\*[\s\S]*?\*\//g, 'tok-com'], [/[a-zA-Z-]+(?=\s*:)/g, 'tok-prop'], [/#[0-9a-fA-F]{3,8}\b/g, 'tok-num'], [/-?\b\d+(?:\.\d+)?(?:px|em|rem|%|vh|vw|s|ms|fr)?\b/gi, 'tok-num'], [/"[^"]*"|'[^']*'/g, 'tok-str']];
...
export function tokenize(line, rules) {
    const out = [];
    let i = 0;
    const n = line.length;
    while (i < n) {
        ...
        for (const r of rules) {
            ...
            if (m && m.index === i && (best === null || m[0].length > best[0].length))
                best = [m[0], r[1]];
        }
        if (best) {
            out.push(best);
...
export function renderLine(line, lang) {

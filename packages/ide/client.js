 ...
 *
 * 资源管理器：目录树、按名查找、新建文件/文件夹、重命名、删除、隐藏文件、语法高亮预览。
 * 源代码管理：stage/unstage（逐文件与全部）、丢弃更改、提交。
 *
...
export default {
  apply(ctx) {
    ...
    const ICONS = {
      ...
      search: [['circle', { cx: 6.8, cy: 6.8, r: 4.4, fill: 'none', stroke: 'currentColor', strokeWidth: 1.4 }], ['path', { d: 'M10.2 10.2 14.2 14.2', fill: 'none', stroke: 'currentColor', strokeWidth: 1.4, strokeLinecap: 'round' }]],
      scm: [['circle', { cx: 3.6, cy: 4, r: 1.5, fill: 'currentColor' }], ['circle', { cx: 12.4, cy: 4, r: 1.5, fill: 'currentColor' }], ['circle', { cx: 12.4, cy: 12, r: 1.5, fill: 'currentColor' }], ['path', { d: 'M3.6 5.5v2.3c0 1.6 1.3 2.9 2.9 2.9h4.3', fill: 'none', stroke: 'currentColor', strokeWidth: 1.4 }], ['path', { d: 'M12.4 5.5v5', fill: 'none', stroke: 'currentColor', strokeWidth: 1.4 }]],
      ...
      newchat: [['path', { d: 'M2 3.5A1.5 1.5 0 0 1 3.5 2h9A1.5 1.5 0 0 1 14 3.5v6a1.5 1.5 0 0 1-1.5 1.5H6.5L3.5 13.5V11A1.5 1.5 0 0 1 2 9.5v-6Z', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2, strokeLinejoin: 'round' }], ['path', { d: 'M8 4.8v4.4M5.8 7h4.4', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2, strokeLinecap: 'round' }]],
      ...
      check: [['path', { d: 'M3 8.5l3 3 7-7', fill: 'none', stroke: 'currentColor', strokeWidth: 1.4, strokeLinecap: 'round', strokeLinejoin: 'round' }]],
      edit: [['path', { d: 'M2.5 13.5v-2.4L10.6 3a1 1 0 0 1 1.4 0l2 2a1 1 0 0 1 0 1.4L5.9 13.5H2.5Z', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2, strokeLinejoin: 'round' }], ['path', { d: 'M9.3 4.3l2.4 2.4', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2 }]],
      trash: [['path', { d: 'M2.5 4.5h11M6.5 4.5V3a1 1 0 0 1 1-1h1a1 1 0 0 1 1 1v1.5M4 4.5l.6 8.5a1 1 0 0 0 1 .9h4.8a1 1 0 0 0 1-.9l.6-8.5', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2, strokeLinecap: 'round', strokeLinejoin: 'round' }], ['path', { d: 'M6.5 7v3.5M9.5 7v3.5', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2, strokeLinecap: 'round' }]],
      file: [['path', { d: 'M4.2 1.6h4.9l3 3v8.8a1 1 0 0 1-1 1H4.2a1 1 0 0 1-1-1V2.6a1 1 0 0 1 1-1Z', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2, strokeLinejoin: 'round' }], ['path', { d: 'M9.1 1.6v3.1h3.1', fill: 'none', stroke: 'currentColor', strokeWidth: 1.2, strokeLinejoin: 'round' }]],
      ...
      refresh: [['path', { d: 'M13.4 8a5.4 5.4 0 1 1-1.6-3.9', fill: 'none', stroke: 'currentColor', strokeWidth: 1.4, strokeLinecap: 'round' }], ['path', { d: 'M12.9 1.9 13.5 4.4l-2.5.6', fill: 'none', stroke: 'currentColor', strokeWidth: 1.4, strokeLinecap: 'round', strokeLinejoin: 'round' }]],
    ...
    function Icon(props) {
      ...
      return el('svg', { viewBox: '0 0 16 16', width: props.size || 16, height: props.size || 16, className: props.className, 'aria-hidden': true, style: Object.assign({ display: 'block', flex: 'none' }, props.style || {}) }, shapes.map(function (s, i) { return el(s[0], Object.assign({ key: i }, s[1])) }))
    }
    ...
    function tokenize(line, rules) {
      ...
      while (i < n) {
        let best = null
        for (const r of rules) { r[0].lastIndex = i; const m = r[0].exec(line); if (m && m.index === i && (best === null || m[0].length > best[0].length)) best = [m[0], r[1]] }
    ...
    function Preview(props) {
      ...
      return el('div', { className: 'dshide-view dshide-preview' },
        ...
        el('pre', { className: 'dshide-code' },
          lines.map(function (ln, i) { return el('div', { key: i, className: 'dshide-codeline' }, el('span', { className: 'dshide-lineno' }, String(i + 1)), renderLine(ln, lang)) }),
    ...
    function Tree(props) {
      ...
      React.useEffect(function () {
        ...
        host.call('ide.listDir', { path: props.path }).then(function (r) { if (cancelled) return; setEntries((r && r.error) ? [] : ((r && r.entries) || [])) })
        return function () { cancelled = true }
      }, [props.path, isOpen])

      const f = (props.filter || '').toLowerCase()
      const visible = (entries || []).filter(function (e) { return (props.showHidden || !(e.name.length > 0 && e.name[0] === '.')) && (f === '' || e.name.toLowerCase().indexOf(f) !== -1) })
      return el('div', null,
          ...
          if (e.type === 'directory') {
            ...
            return el('div', { key: e.path },
              el('div', { className: 'dshide-row', style: { paddingLeft: (props.depth * 12 + 8) + 'px' }, onClick: function () { props.toggle(e.path) } },
                ...
                el('span', { className: 'dshide-row-actions', onClick: function (ev) { ev.stopPropagation() } },
                  el('button', { type: 'button', className: 'dshide-row-btn', title: '重命名', onClick: function (ev) { ev.stopPropagation(); props.onRename(e.path, e.name) } }, el(Icon, { name: 'edit', size: 13 })),
                  el('button', { type: 'button', className: 'dshide-row-btn', title: '删除', onClick: function (ev) { ev.stopPropagation(); props.onDelete(e.path, e.name) } }, el(Icon, { name: 'trash', size: 13 })),
                ),
              ),
              open ? el(Tree, { path: e.path, depth: props.depth + 1, expanded: props.expanded, toggle: props.toggle, onOpen: props.onOpen, showHidden: props.showHidden, filter: props.filter, onRename: props.onRename, onDelete: props.onDelete }) : null,
            )
          }
    ...
    function ExplorerView(props) {
      ...
      function openFile(path) { setPreview({ path: path, loading: true }); host.call('ide.readText', { path: path }).then(function (r) { setPreview({ path: path, content: r && r.content, error: r && r.error, truncated: r && r.truncated }) }) }
      ...
      return el('div', { className: 'dshide-view' },
        el('div', { className: 'dshide-toolbar' },
          el('select', { className: 'dshide-select', value: props.root || '', onChange: function (e) { props.setRoot(e.target.value); refresh(); setPreview(null) } }, options.map(function (o) { return el('option', { key: o.path, value: o.path }, o.title) })),
          el('button', { type: 'button', className: 'dshide-iconbtn', title: '新建文件', onClick: function () { startAction('newfile', props.root, '') } }, el(Icon, { name: 'file', size: 14 })),
          el('button', { type: 'button', className: 'dshide-iconbtn', title: '新建文件夹', onClick: function () { startAction('newdir', props.root, '') } }, el(Icon, { name: 'folder', size: 14 })),
          el('button', { type: 'button', className: 'dshide-iconbtn', title: '查找', onClick: function () { setFilter('') } }, el(Icon, { name: 'search', size: 14 })),
          el('button', { type: 'button', className: 'dshide-iconbtn', title: '刷新', onClick: refresh }, el(Icon, { name: 'refresh', size: 14 })),
          el('button', { type: 'button', className: 'dshide-iconbtn', title: '显示隐藏文件', onClick: function () { setShowHidden(function (v) { return !v }) }, style: showHidden ? { color: 'var(--dsw-alias-brand-primary)' } : undefined }, el(Icon, { name: 'file', size: 14 })),
        ),
        el('input', { className: 'dshide-search-input', style: { margin: '6px 8px', width: 'auto', flex: 'none' }, placeholder: '按名称查找…', value: filter, onChange: function (e) { setFilter(e.target.value) } }),
        action ? el('div', { className: 'dshide-actionbar' },
          el('span', { className: 'dshide-actionbar-label' }, actionLabel),
          action.kind !== 'delete' ? el('input', { className: 'dshide-actionbar-input', autoFocus: true, value: input, onChange: function (e) { setInput(e.target.value) }, onKeyDown: function (e) { if (e.key === 'Enter') runAction(); if (e.key === 'Escape') cancel() } }) : null,
          el('button', { type: 'button', className: 'dshide-iconbtn', title: '确认', onClick: runAction, disabled: busy }, el(Icon, { name: action.kind === 'delete' ? 'trash' : 'check', size: 14 })),
          el('button', { type: 'button', className: 'dshide-iconbtn', title: '取消', onClick: cancel }, el(Icon, { name: 'close', size: 14 })),
        ) : null,
        el('div', { className: 'dshide-scroll' },
          props.root ? el(Tree, { path: props.root, depth: 0, expanded: expanded, toggle: toggle, onOpen: openFile, showHidden: showHidden, filter: filter, onRename: function (p, n) { startAction('rename', p, n) }, onDelete: function (p, n) { startAction('delete', p, n) } }) :
    ...
    function SearchView(props) {
      ...
      function run() { if (!query.trim()) return; setLoading(true); setResult(null); host.call('ide.search', { cwd: props.root, query: query, caseSensitive: cs }).then(function (r) { setResult(r || { error: 'no response', matches: [], files: 0, truncated: false }); setLoading(false) }) }
      function openFile(path) { setPreview({ path: path, loading: true }); host.call('ide.readText', { path: path }).then(function (r) { setPreview({ path: path, content: r && r.content, error: r && r.error, truncated: r && r.truncated }) }) }
      ...
      return el('div', { className: 'dshide-view' },
        ...
        el('div', { className: 'dshide-results' },
          ...
          result.matches.map(function (m, i) { return el('div', { key: i, className: 'dshide-match', onClick: function () { openFile(m.path) } }, el('div', { className: 'dshide-match-path' }, m.path), el('div', { className: 'dshide-match-line' }, el('span', { className: 'dshide-match-lineno' }, String(m.line)), el('span', { className: 'dshide-match-text' }, m.text))) }),
        ),
      )
    }
    ...
    function DiffView(props) {
      ...
      return el('div', { className: 'dshide-view dshide-preview' },
        ...
        el('pre', { className: 'dshide-code' }, lines.map(function (ln, i) { const cls = ln.indexOf('+') === 0 && ln.indexOf('+++') !== 0 ? 'dshide-diff-line add' : ln.indexOf('-') === 0 && ln.indexOf('---') !== 0 ? 'dshide-diff-line del' : ln.indexOf('@@') === 0 ? 'dshide-diff-line hunk' : 'dshide-diff-line'; return el('div', { key: i, className: cls }, ln || ' ') })),
      )
    }
    ...
    function ScmView(props) {
      ...
      function refresh() { if (!props.root) return; setLoading(true); host.call('ide.git.status', { cwd: props.root }).then(function (r) { setStatus(r || { branch: '', changes: [], notRepo: true, error: 'no response' }); setLoading(false) }) }
      ...
      function openDiff(path) { setDiff({ path: path, loading: true }); host.call('ide.git.diff', { cwd: props.root, path: path }).then(function (r) { setDiff({ path: path, stdout: r && r.stdout, stderr: r && r.stderr, ok: r && r.ok }) }) }
      function commit() { if (!message.trim()) return; setBusy(true); host.call('ide.git.commit', { cwd: props.root, message: message }).then(function (r) { setBusy(false); setMessage(''); refresh() }) }
      ...
      function row(c, action, iconName) {
        return el('div', { key: c.path, className: 'dshide-row', onClick: function () { openDiff(c.path) } },
          ...
          el('span', { className: 'dshide-row-actions', onClick: function (ev) { ev.stopPropagation() } },
            el('button', { type: 'button', className: 'dshide-row-btn', title: action === 'stage' ? '暂存' : '取消暂存', onClick: function (ev) { ev.stopPropagation(); act(action === 'stage' ? 'ide.git.stage' : 'ide.git.unstage', { cwd: props.root, paths: [c.path] }) } }, el(Icon, { name: action === 'stage' ? 'plus' : 'minus', size: 13 })),
            el('button', { type: 'button', className: 'dshide-row-btn', title: '丢弃更改', onClick: function (ev) { ev.stopPropagation(); act('ide.git.discard', { cwd: props.root, path: c.path, untracked: c.xy === '??' }) } }, el(Icon, { name: 'trash', size: 13 })),
          ),
        )
      }
      ...
      return el('div', { className: 'dshide-view' },
        el('div', { className: 'dshide-toolbar' },
          el('span', { className: 'dshide-branch' }, el(Icon, { name: 'scm', size: 14 }), el('span', null, (status && status.branch) || '源代码管理')),
          el('button', { type: 'button', className: 'dshide-iconbtn', title: '暂存全部', onClick: function () { act('ide.git.stageAll', { cwd: props.root }) }, disabled: busy }, el(Icon, { name: 'plus', size: 14 })),
          el('button', { type: 'button', className: 'dshide-iconbtn', title: '取消暂存全部', onClick: function () { act('ide.git.unstageAll', { cwd: props.root }) }, disabled: busy }, el(Icon, { name: 'minus', size: 14 })),
    ...
    function SessionView(props) {
      ...
      const rows = ids.map(function (id) { return byId[id] }).filter(function (s) { return s && s.origin !== 'subagent' && !archived.has(s.id) && (!s.blank || s.id === current) }).sort(function (a, b) { return b.updatedAt - a.updatedAt })

      function open(id) { const sessions = ctx.get('sessions'); if (sessions) sessions.open(id) }
      function newSession() { const ws = ctx.get('workspaces'); if (ws) ws.startSession() }
      function addWorkspace() { const ws = ctx.get('workspaces'); if (ws) ws.pickDirectory().then(function (path) { if (path) ws.create({ path: path }) }) }

      function dotClass(s) { if (s.pendingInteraction) return 'dshide-dot dshide-dot-warn'; if (s.running) return 'dshide-dot dshide-dot-ongoing'; if (s.completed) return 'dshide-dot dshide-dot-done'; return 'dshide-dot dshide-dot-idle' }
      ...
      return el('div', { className: 'dshide-view' },
        ...
        el('div', { className: 'dshide-scroll' },
          ...
          rows.map(function (s) { const wsTitle = workspaceBySession[s.id]; return el('div', { key: s.id, className: 'dshide-row' + (s.id === current ? ' selected' : ''), onClick: function () { open(s.id) } }, el('span', { className: dotClass(s) }), el('span', { className: 'dshide-name' }, s.displayTitle || s.id), wsTitle ? el('span', { className: 'dshide-rename', title: wsTitle }, wsTitle) : null, el('span', { className: 'dshide-time' }, relTime(s.updatedAt))) }),
        ),
      )
    }

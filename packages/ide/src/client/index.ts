/**
 * deepseek-harness-UI — Client half: the unified sidebar (Explorer / Search /
 * Source Control / Sessions) and the editor column.
 *
 * Registers into `sidebar.workspaces` (priority -1) and, when the layout has
 * the editor column (`ctx.layout.openEditor`), into `editor`; otherwise it
 * falls back to a `conversation.view` tab. Filesystem / git / search primitives
 * come from the Host `ide` Remote namespace, mounted by this client itself.
 */
...
async function rpc<T>(p: Promise<RemoteResult<T>>): Promise<T> {
  const r = await p
  if (!r.ok) throw new Error(r.error.message)
  return r.value
}

// Module-scope CSS injection (removed by the loader on unload via data-plugin).
...
export async function apply(ctx: Context): Promise<void> {
  ...
  const P = (d: string): { d: string; fill: string } => ({ d, fill: 'currentColor' })

  const ICONS: Record<string, { vb: string; p: Array<[string, Record<string, unknown>]> }> = {
  ...
  function Icon(props: { name: string; size?: number; className?: string; style?: Record<string, unknown> }): ReturnType<typeof el> | null {
    const def = ICONS[props.name]
    if (!def) return null
    return el('svg', { viewBox: def.vb, width: props.size ?? 16, height: props.size ?? 16, className: props.className, 'aria-hidden': true, fill: 'none', style: Object.assign({ display: 'block', flex: 'none' }, props.style ?? {}) }, def.p.map((s, i) => el(s[0], Object.assign({ key: i }, s[1]))))
  }

  const MATRIX_CELLS = [[0, 0], [4, 0], [8, 0], [8, 4], [8, 8], [4, 8], [0, 8], [0, 4]]
  function StateDot(props: { state?: string; size?: number }): ReturnType<typeof el> {
    const state = props.state ?? 'idle'
    const size = props.size ?? 10
    if (state === 'ongoing') {
      return el('svg', { className: 'dshide-dot-matrix', width: size, height: size, viewBox: '0 0 10 10', shapeRendering: 'crispEdges', 'aria-hidden': true }, MATRIX_CELLS.map((c, i) => el('rect', { key: `${c[0]}-${c[1]}`, className: 'dshide-dot-cell', x: c[0], y: c[1], width: 2, height: 2, style: { animationDelay: `${(i - 8) * 125}ms` } })))
    }
    return el('span', { className: `dshide-dot dshide-dot-${state}`, style: { width: size, height: size }, 'aria-hidden': true })
  }
  ...
  const dirnameOf = (p: string): string => { const i = Math.max(p.lastIndexOf('\\'), p.lastIndexOf('/')); return i < 0 ? p : p.slice(0, i) }
  const baseName = (p: string): string => { const s = p.replace(/[\\/]+$/, ''); const i = Math.max(s.lastIndexOf('\\'), s.lastIndexOf('/')); return i < 0 ? s : s.slice(i + 1) }
  ...
  function tokenize(line: string, rules: Array<[RegExp, string]>): Array<[string, string | null]> {
  ...
  function renderLine(line: string, lang: string): ReturnType<typeof el> {
    const segs = tokenize(line, buildRules(lang))
    return el('span', { className: 'dshide-linetext' }, segs.map((s, i) => (s[1] ? el('span', { key: i, className: s[1] }, s[0]) : s[0])))
  }

  // Module-level document/diff tab list shared between the sidebar and the editor column.
  const docStore = {
    tabs: [] as Array<{ key: string; kind: 'file' | 'diff'; path: string; cwd?: string }>,
    ...
    emit(): void { for (const fn of this.listeners) { try { fn() } catch { /* listener isolated */ } } },
    add(tab: { key: string; kind: 'file' | 'diff'; path: string; cwd?: string }): void {
      const ex = this.tabs.find((t) => t.key === tab.key)
      if (ex) { this.activeId = tab.key; this.emit(); return }
      this.tabs.push(tab); this.activeId = tab.key; this.emit()
    },
    close(key: string): void {
      const i = this.tabs.findIndex((t) => t.key === key)
      if (i < 0) return
      this.tabs.splice(i, 1)
      if (this.activeId === key) { const next = this.tabs[i] ?? this.tabs[i - 1]; this.activeId = next ? next.key : null }
      this.emit()
    },
    setActive(key: string): void { if (this.activeId !== key) { this.activeId = key; this.emit() } },
  }
  ...
  function openDoc(tab: { key: string; kind: 'file' | 'diff'; path: string; cwd?: string }): void {
    docStore.add(tab)
    if (hasEditorColumn) { layout?.openEditor?.(); return }
    clickTabNow(['编辑器', 'Editor'])
  }

  function FileEditor(props: { path: string }): ReturnType<typeof el> {
    const [st, setSt] = useState<{ loading: boolean; content?: string; error?: string; truncated?: boolean }>({ loading: true })
    useEffect(() => {
      let cancelled = false
      setSt({ loading: true })
      rpc(ide.readText(props.path)).then((r) => { if (!cancelled) setSt({ content: r.content, error: '', truncated: r.truncated, loading: false }) }, (e) => { if (!cancelled) setSt({ error: String((e as Error).message), loading: false }) })
  ...
  function DiffEditor(props: { cwd: string; path: string }): ReturnType<typeof el> {
    const [st, setSt] = useState<{ loading: boolean; stdout?: string; stderr?: string }>({ loading: true })
    useEffect(() => {
      let cancelled = false
      setSt({ loading: true })
      rpc(ide.gitDiff(props.cwd, props.path)).then((r) => { if (!cancelled) setSt({ stdout: r.stdout, stderr: r.stderr, loading: false }) }, (e) => { if (!cancelled) setSt({ stderr: String((e as Error).message), loading: false }) })
      return () => { cancelled = true }
  ...
  function EditorView(): ReturnType<typeof el> {
    const [state, setState] = useState<{ tabs: typeof docStore.tabs; activeId: string | null }>({ tabs: docStore.tabs, activeId: docStore.activeId })
    useEffect(() => docStore.subscribe(() => { setState({ tabs: docStore.tabs, activeId: docStore.activeId }) }), [])
    const tabs = state.tabs
    const activeId = state.activeId
    const active = tabs.find((t) => t.key === activeId) ?? null
    return el('div', { className: 'dshide-editor' }, el('div', { className: 'dshide-etabs' }, tabs.map((t) => el('div', { key: t.key, className: `dshide-etab${t.key === activeId ? ' active' : ''}`, title: t.path, onClick: () => { docStore.setActive(t.key) } }, el('span', { className: 'dshide-etab-label' }, baseName(t.path) + (t.kind === 'diff' ? ' ⇄' : '')), el('button', { type: 'button', className: 'dshide-etab-close', title: '关闭', onClick: (e: { stopPropagation: () => void }) => { e.stopPropagation(); docStore.close(t.key) } }, el(Icon, { name: 'close', size: 11 })))), tabs.length === 0 ? el('span', { className: 'dshide-etab-hint' }, '从侧栏打开文件或 diff') : null), active === null ? el('div', { className: 'dshide-editor-empty' }, '在资源管理器 / 搜索 / 源码管理中打开文档') : active.kind === 'file' ? el(FileEditor, { path: active.path }) : el(DiffEditor, { cwd: active.cwd ?? '', path: active.path })))
  }

  function Tree(props: { path: string; depth: number; expanded: Set<string>; toggle: (p: string) => void; onOpen: (p: string) => void; showHidden: boolean; filter: string; onRename: (p: string, n: string) => void; onDelete: (p: string, n: string) => void; onMove: (src: string, dest: string) => void }): ReturnType<typeof el> {
    const isOpen = props.depth === 0 || props.expanded.has(props.path)
    const [entries, setEntries] = useState<Array<{ name: string; type: string; path: string; size: number | null }> | undefined>(undefined)
    useEffect(() => {
      let cancelled = false
    ...
    return el('div', null, entries === undefined ? el('div', { className: 'dshide-loading' }, '加载中…') : visible.map((e) => {
      if (e.type === 'directory') {
        ...
        }, el('span', { className: `dshide-arrow${open ? ' open' : ''}` }, el(Icon, { name: 'chevron', size: 12 })), el(Icon, { name: 'folder', size: 15, className: 'dshide-glyph' }), el('span', { className: 'dshide-name' }, e.name), el('span', { className: 'dshide-row-actions', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation() } }, el('button', { type: 'button', className: 'dshide-row-btn', title: '在资源管理器中打开', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation(); void rpc(ide.explore(e.path, false)) } }, el(Icon, { name: 'locate', size: 13 })), el('button', { type: 'button', className: 'dshide-row-btn', title: '重命名', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation(); props.onRename(e.path, e.name) } }, el(Icon, { name: 'edit', size: 13 })), el('button', { type: 'button', className: 'dshide-row-btn', title: '删除', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation(); props.onDelete(e.path, e.name) } }, el(Icon, { name: 'trash', size: 13 })))), open ? el(Tree, { path: e.path, depth: props.depth + 1, expanded: props.expanded, toggle: props.toggle, onOpen: props.onOpen, showHidden: props.showHidden, filter: props.filter, onRename: props.onRename, onDelete: props.onDelete, onMove: props.onMove }) : null)
      }
      return el('div', {
  ...
  function ExplorerView(props: { root?: string; setRoot: (p: string) => void; workspaces: Array<{ path: string; title?: string }> }): ReturnType<typeof el> {
  ...
  function ScmView(props: { root?: string }): ReturnType<typeof el> {
    const [status, setStatus] = useState<{ branch: string; changes: Array<{ xy: string; path: string; renameFrom: string; staged: string; unstaged: string }>; notRepo: boolean; error: string } | null>(null)
    const [loading, setLoading] = useState(false)
  ...
  function SessionView(props: { wsState: any; sessState: any }): ReturnType<typeof el> {
    const wsState = props.wsState
    const sessState = props.sessState
    const ids: string[] = sessState?.ids ?? []
    const byId: Record<string, any> = sessState?.byId ?? {}
    const current: string | undefined = sessState?.current
    const workspaces: Array<{ workspaceId: string; title: string; path: string; sessionIds: string[] }> = wsState?.items ?? []
    const archived = new Set<string>(wsState?.archivedSessionIds ?? [])
    const sessionsSvc = ctx.get('sessions') as any
    const workspacesSvc = ctx.get('workspaces') as any
    ...
    useEffect(() => {
      ...
      const timer = window.setTimeout(() => {
        if (sessionsSvc?.search) sessionsSvc.search(q, ctrl.signal).then((r: any) => { if (!cancelled) setResults(r?.items ? r : { items: [], hasMore: false }) }).catch(() => { if (!cancelled) setResults({ items: [], hasMore: false }) })
    ...
    const visible = (s: any): boolean => s && s.origin !== 'subagent' && !archived.has(s.id) && (!s.blank || s.id === current)
    const label = (s: any): string => workspaceBySession[s.id] || (s.cwd ? s.cwd.replace(/[\\/]+$/, '').split(/[\\/]/).pop() : '')
    const open = (id: string): void => {
      sessionsSvc?.open(id)
      if (id === current) { clickChatTab() } else { [150, 350, 650, 1100].forEach((d) => { window.setTimeout(clickChatTab, d) }) }
    }
    const newSession = (): void => { workspacesSvc?.startSession() }
    const addWorkspace = (): void => { workspacesSvc?.pickDirectory().then((p: string | null) => { if (p) workspacesSvc.create({ path: p }) }) }
    const forkSession = (id: string): void => { sessionsSvc?.fork({ sessionId: id, increaseTitle: true }).then((cid: string) => { if (cid) sessionsSvc?.open(cid) }) }
    const archiveSession = (id: string): void => { workspacesSvc?.archiveSession(id) }
    const toggleGroup = (k: string): void => { setExpanded((prev) => { const n = new Set(prev); if (n.has(k)) n.delete(k); else n.add(k); return n }) }
    const runAction = (): void => {
      const a = action
      if (!a) return
      const done = (): void => { setAction(null); setInput('') }
      if (a.kind === 'wrename' && input.trim()) workspacesSvc.rename(a.id, input.trim()).then(done)
      else if (a.kind === 'wdelete') workspacesSvc.delete(a.id).then(done)
    }
    const sessionState = (s: any): string => { if (s.pendingInteraction) return 'warning'; if (s.running) return 'ongoing'; if (s.completed) return 'done'; return 'idle' }
    const sessionRow = (s: any, indent: boolean): ReturnType<typeof el> => el('div', { key: s.id, className: `dshide-row${s.id === current ? ' selected' : ''}`, style: indent ? { paddingLeft: '18px' } : undefined, title: s.displayTitle, onClick: () => { open(s.id) } }, el(StateDot, { state: sessionState(s), size: 10 }), el('span', { className: 'dshide-name' }, s.displayTitle || s.id), el('span', { className: 'dshide-time' }, relTime(s.updatedAt)), el('span', { className: 'dshide-row-actions', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation() } }, el('button', { type: 'button', className: 'dshide-row-btn', title: '派生会话', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation(); forkSession(s.id) } }, el(Icon, { name: 'scm', size: 13 })), el('button', { type: 'button', className: 'dshide-row-btn', title: '归档', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation(); archiveSession(s.id) } }, el(Icon, { name: 'trash', size: 13 }))))
    const workspaceRow = (w: typeof workspaces[number]): ReturnType<typeof el> => {
      const members = (w.sessionIds ?? []).map((id) => byId[id]).filter(visible).sort((a: any, b: any) => b.updatedAt - a.updatedAt)
      const isOpen = expanded.has(w.workspaceId)
      return el('div', { key: w.workspaceId }, el('div', { className: 'dshide-wsgroup-title', title: w.path, onClick: () => { toggleGroup(w.workspaceId) } }, el('span', { className: `dshide-arrow${isOpen ? ' open' : ''}` }, el(Icon, { name: 'chevron', size: 12 })), el(Icon, { name: 'folder', size: 14, className: 'dshide-glyph' }), el('span', { className: 'dshide-name' }, w.title), el('span', { className: 'dshide-row-actions', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation() } }, el('button', { type: 'button', className: 'dshide-row-btn', title: '重命名工作区', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation(); setAction({ kind: 'wrename', id: w.workspaceId, name: w.title }); setInput(w.title) } }, el(Icon, { name: 'edit', size: 13 })), el('button', { type: 'button', className: 'dshide-row-btn', title: '删除工作区', onClick: (ev: { stopPropagation: () => void }) => { ev.stopPropagation(); setAction({ kind: 'wdelete', id: w.workspaceId, name: w.title }) } }, el(Icon, { name: 'trash', size: 13 })))), isOpen ? members.map((s: any) => sessionRow(s, true)) : null)
    }
    let body: ReturnType<typeof el>
    if (q !== '') {
      const local = ids.map((id) => byId[id]).filter((s) => visible(s) && (s.displayTitle.toLowerCase().includes(q) || label(s).toLowerCase().includes(q))).sort((a: any, b: any) => b.updatedAt - a.updatedAt)
      const content: any[] = results?.items ?? []
      const merged: any[] = local.map((s) => ({ id: s.id, title: s.displayTitle, ws: label(s), s }))
      const seenIds: Record<string, boolean> = {}
      merged.forEach((m) => { seenIds[m.id] = true })
      content.forEach((c) => { if (!seenIds[c.sessionId] && byId[c.sessionId]) merged.push({ id: c.sessionId, title: byId[c.sessionId].displayTitle, ws: label(byId[c.sessionId]), snippet: c.snippet, s: byId[c.sessionId] }) })
      body = merged.slice(0, 20).map((m) => el('div', { key: m.id, className: 'dshide-row', onClick: () => { open(m.id) } }, el(StateDot, { state: sessionState(m.s), size: 10 }), el('span', { className: 'dshide-name' }, m.title), m.ws ? el('span', { className: 'dshide-rename' }, m.ws) : null, m.snippet ? el('span', { className: 'dshide-time' }, m.snippet) : null))
      if (body.length === 0) body = el('div', { className: 'dshide-empty' }, '未找到匹配的会话。')
    } else if (view === 'flat') {
      const flat = ids.map((id) => byId[id]).filter(visible).sort((a: any, b: any) => b.updatedAt - a.updatedAt)
      body = flat.length === 0 ? el('div', { className: 'dshide-empty' }, '暂无会话。') : flat.map((s) => sessionRow(s, false))
    } else {
      const accounted = new Set<string>()
      const groups = workspaces.map((w) => { w.sessionIds.forEach((id) => { accounted.add(id) }); return workspaceRow(w) })
      const stray = ids.map((id) => byId[id]).filter((s) => visible(s) && !accounted.has(s.id)).sort((a: any, b: any) => b.updatedAt - a.updatedAt)
  ...
  function IdeRegion(props: { wide: boolean; expandSidebar?: () => void; useWorkspaces: any; useSessions: any }): ReturnType<typeof el> {
    const wsState = props.useWorkspaces ? props.useWorkspaces((s: any) => s) : null
    const sessState = props.useSessions ? props.useSessions((s: any) => s) : null
    const [active, setActive] = useState('sessions')
    const [root, setRoot] = useState<string | undefined>(undefined)
    const items: Array<{ workspaceId: string; title?: string; path: string }> = wsState?.items ?? []
    ...
    useEffect(() => {
      ...
      rpc(ide.roots()).then((r) => { if (r.root) setRoot(r.root); else if (r.workspaces[0]) setRoot(r.workspaces[0].path) })
    }, [root, recent, items])
    ...
    const buttons = (): ReturnType<typeof el> => views.map((v) => el('button', { key: v.id, type: 'button', title: v.label, 'aria-label': v.label, className: `dshide-activity-btn${props.wide && active === v.id ? ' active' : ''}`, onClick: () => { pick(v.id) } }, el(Icon, { name: v.icon, size: 20 })))
    if (!props.wide) return el('div', { className: 'dshide-region rail' }, buttons())
    return el('div', { className: 'dshide-region' }, el('div', { className: 'dshide-activity' }, buttons()), el('div', { className: 'dshide-content' }, active === 'sessions' ? el(SessionView, { wsState, sessState }) : active === 'files' ? el(ExplorerView, { root, setRoot, workspaces: items }) : active === 'search' ? el(SearchView, { root }) : el(ScmView, { root })))
  }

  slots.inject('sidebar.workspaces', () => slots.register({ name: 'sidebar.workspaces', priority: -1 }, IdeRegion as any))

  if (hasEditorColumn) {
    slots.inject('editor', () => slots.register({ name: 'editor' }, EditorView as any))
  } else {
    slots.inject('conversation.view', () => slots.register({ name: 'conversation.view', id: 'editor', order: 20, label: '编辑器' }, EditorView as any))
  }
}
